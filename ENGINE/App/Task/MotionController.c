...
/* function prototypes -------------------------------------------------------*/
 /* 三次样条插值中间变量 */
	...
	float* jointSpeedLimit[JointAmount];          //关节速度上限指针（m/s 或 rad/s）
 ...
 * 
 */
void MotionControllerClassdef(void)
{
  /* 初始化轨迹点数量为0 */
  pointNum = 0;
  /* 把对角线全部设置为2 */
 ...
 */
void receiveTracjectory(int _pointNum,float* _timefromStart,float _JointsPosition[JointAmount][MaxPointAmount],float _JointsVelocity[JointAmount][MaxPointAmount],bool _useCubic) 
{
  if(!interOK)
  {  /* 记录轨迹点数量 */
    pointNum = _pointNum;
  ...
  if(_tempxVariable <= timefromStart[curveNO+1])  //目标位于当前曲线
  {
    ...
    {   //第i个关节
      tempDeltaJointTarget = (useCubic?(jointCubeInterCoe[i].FirstCoe[curveNO]*pow((timefromStart[curveNO+1] - _tempxVariable),3) + jointCubeInterCoe[i].SecCoe[curveNO]*pow((_tempxVariable - timefromStart[curveNO]),3) + jointCubeInterCoe[i].ThirdCoe[curveNO]*(timefromStart[curveNO+1] - _tempxVariable) + jointCubeInterCoe[i].FourthCoe[curveNO]*(_tempxVariable - timefromStart[curveNO]))
        ...
        - jointTargetptr[i];
      if((fabs(tempDeltaJointTarget)/tskCyclic)*1000.0f > *jointSpeedLimit[i])
      {
        return true;
      }
    }
